<template>
  <v-col class="mb-4">
    <h3 class="display-2 font-weight-bold mb-3">Feature List</h3>
    <v-table>
      <thead>
        <tr>
          <th>Select</th>
...
import type { IFeatureForm } from '@/interfaces/main'
// Raw data
...
export default defineComponent({
  name: 'FormFeature',
  mounted(): void {
    this.form = data.map(({ sub_type, name, resources }) => {
      return {
    ...
    this.selected_sub_type_list = [
      ...new Set(data.map(({ sub_type }) => this.capitalized(sub_type))),
      'All'
    ]
  },
  data() {
  ...
  methods: {
    ...
    submit(): void {
      const featuresTmp = this.form.filter(({ selected }) => selected === true)
